...
static bool isNop(Stm* x) {
    return typeid(*x) == typeid(ExpStm)
           && (typeid(*static_cast<ExpStm*>(x)->exp) == typeid(ConstInt)
               || typeid(*static_cast<ExpStm*>(x)->exp) == typeid(ConstFloat));
}
...
static Stm* do_stm(Stm* stm) {
    if (typeid(*stm) == typeid(Seq)) {
        ...
        if (typeid(*(s->dst)) == typeid(Temp) && typeid(*(s->src)) == typeid(Call))
            ...
            return seq(
                reorder(new ExpRefList(&static_cast<Mem*>(s->dst)->mem, new ExpRefList(&s->src, NULL))),
                stm);
