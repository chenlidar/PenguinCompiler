...
using std::string;
using std::vector;
typedef C_stmListList_* C_stmListList;
struct C_block {
...
vector<Stm> C_linearize(Stm stm);
...
struct C_block C_basicBlocks(vector<Stm> stmList);
...
vector<Stm> C_traceSchedule(struct C_block b);
